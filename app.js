...
app.get("/dashboard/", AccessMiddleware.isLoggedIn, (req, res) => {
  ...
  enfant.find({location: req.user.location}, (err, total) => {
    total.forEach((iteration) => {
...
app.post("/create", AccessMiddleware.isLoggedIn, AccessMiddleware.isSuper, upload.single("photo_enfant"), async(req, res) => {
  ...
  if (req.body.cantine) {
    ...
    else if (req.body.cantine == "2") enfant_obj.type_cantine = 2;
    else if (req.body.cantine == "3") enfant_obj.type_cantine = 3;
  }
  if (!req.body.cantine) {
